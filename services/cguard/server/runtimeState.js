...
class RuntimeState {
  ...
  listSessions({
    ...
    const latestUploadStatusBySession = new Map();
    for (let index = this.events.length - 1; index >= 0; index -= 1) {
      const event = this.events[index];
      if (!isPlainObject(event)) continue;
      if (event.event_type !== "CLIENT_UPLOAD_STATUS") continue;
      const sessionId =
        typeof event.session_id === "string" ? event.session_id.trim() : "";
      if (!sessionId || latestUploadStatusBySession.has(sessionId)) continue;
      const evidence = isPlainObject(event.evidence) ? event.evidence : {};
      latestUploadStatusBySession.set(sessionId, {
        status:
      ...
      if (latestUploadStatusBySession.size >= this.sessionsById.size) {
        break;
      }
    }
    ...
    const sessionRows = Array.from(this.sessionsById.values()).map((session) => {
      ...
      if (session.ended_at || !this.isClientOnline(session.client_instance_id)) {
