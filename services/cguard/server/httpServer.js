...
function collectCliEvidenceTexts(event) {
  ...
  push(evidence.remote_host);
  push(evidence.remote_hostname);
...
function createApp(options = {}) {
  ...
  const state = options.state || createStateAdapter(options);
  ...
  async function handleEvents(req, res) {
    ...
    if (decision.status === "blocked") {
      if (issueUserBanOnBlockedDecision) {
        state.setUserBan(session.user_id, 600, "BANNED_USER", decision.message);
      }
      state.revokeActiveJtiForUser(session.user_id);
    }
    state.setSessionDecision(session.session_id, {
