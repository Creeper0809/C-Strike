...
def _problem_list_embed(problems: list[dict[str, Any]]) -> discord.Embed:
    ...
    if not problems:
        embed.add_field(name="안내", value="현재 배포된 문제가 없습니다.", inline=False)
        return embed
...
def _problem_detail_embed(problem: dict[str, Any], team_links: list[dict[str, Any]]) -> discord.Embed:
    ...
    connection_info = str(problem.get("connection_info") or "").strip()
    embed = discord.Embed(
    ...
    if connection_info:
        embed.add_field(name="🔹 접속 안내", value=connection_info[:1024], inline=False)

    if not team_links:
        embed.add_field(name="✨ 팀별 링크", value="현재 배포된 팀 링크가 없습니다.", inline=False)
        return embed
...
class ProblemCog(commands.Cog):
    ...
    @app_commands.command(name="문제목록", description="현재 배포된 문제 목록을 조회합니다.")
    ...
    @app_commands.command(name="문제", description="문제 번호로 배포된 문제 상세 정보와 팀별 링크를 조회합니다.")
    @app_commands.describe(problem_no="예: WEB-001")
    async def problem_detail(self, interaction: discord.Interaction, problem_no: str) -> None:
        ...
        if problem is None:
            await interaction.followup.send("해당 문제를 찾을 수 없거나 아직 배포되지 않았습니다.", ephemeral=True)
            return
