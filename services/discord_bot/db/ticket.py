...
def next_ticket_sequence() -> int:
    return _counter_next("ticket_id")


def allocate_ticket_identity(category: str) -> tuple[str, str]:
    ticket_seq = _counter_next("ticket_id")
    ticket_id = f"TKT-{ticket_seq:06d}"

    prefix = _category_prefix(category)
    number_seq = _counter_next(f"ticket_number:{prefix}")
    ticket_number = f"{prefix}-{number_seq:06d}"
...
def assign_ticket_number_for_category(ticket_id: str, category: str) -> str:
    ...
    prefix = _category_prefix(category)
    current_number = str(row.get("ticket_number") or "").strip().lower()
    if current_number.startswith(f"{prefix}-"):
        return current_number

    seq = _counter_next(f"ticket_number:{prefix}")
...
def store_ticket(
    ...
    if not safe_ticket_id:
        raise ValueError("ticket_id is required")
    safe_ticket_number = str(ticket_number or "").strip().lower()
